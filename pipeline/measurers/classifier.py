...
class ClassifierMeasurer(BaseMeasurer):
    ...
    def _load_onnx(self, path: Path) -> None:
        ...
        self._backend = "onnx"
        logger.info(f"ONNX classifier loaded successfully for target '{self.target}'")
    ...
    def _load_torch(self, path: Path) -> None:
        ...
        self._backend = "torch"
        logger.info(f"PyTorch classifier loaded successfully for target '{self.target}'")
    ...
    def measure(self, roi: np.ndarray) -> MeasurementResult:
        ...
        raw_labels = getattr(target_cfg, "labels", None)
        if not raw_labels or isinstance(raw_labels, DotMap):
            raise ValueError(
                f"No 'labels' list defined in config.yaml under measurers.{self.target}. "
                f"Add a 'labels' key with your model's class names."
            )
